...
sudo $jps_cmd -v | grep -Ev '^$|Jps' | grep jboss-modules |
while read psline; do 
  ...
  sudo $jinfo_cmd $pid 2>&1|grep -E '^jboss.server.log.dir|^jboss.server.base.dir|^org.jboss.boot.log.file'| sort|awk '{print "\t"$3}'
  echo ""
  sudo netstat -antpuew | grep -E "$pid/java" | grep LISTEN | awk '{print "\t"$1" "$4}' | sort
  echo ""
done
