...
if [ ! -z $PATTERN  ] ; then
    # string
    PROCESS_ID=`ps -ef |egrep -v 'grep|threads_per' |grep $1|gawk '{print $2}'`
    echo "The PID to lookup is: "$PROCESS_ID
fi

CMD="ps -p $PROCESS_ID -o pid,user,%cpu,rss,etime,nlwp,args"
CMD2="ps -p $PROCESS_ID h -o pid,user,%cpu,rss,etime,nlwp,args"
